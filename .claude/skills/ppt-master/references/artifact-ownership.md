...
| `analysis/delegates/*.md`, `analysis/delegates/*.json` | Subagent report channel | Findings a delegated step returns instead of pasting into the conversation — source digest, template preflight, image acquisition, deck / chart / export verification | Written by the delegate named in [`subagent-delegation.md`](./subagent-delegation.md) §1; the main agent reads one only when it needs detail beyond the returned summary. Never a design contract and never a substitute for the artifact it describes |
...
| Delegate reports | `analysis/delegates/` is a derived report channel. Regenerate it by re-running the delegate; never edit a report to change a verdict, and never let one stand in for `design_spec.md`, `spec_lock.md`, or the source file it summarizes. |
