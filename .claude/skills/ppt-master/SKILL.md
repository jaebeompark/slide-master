...
| Subagent delegation | [`references/subagent-delegation.md`](references/subagent-delegation.md) | Owns which steps delegate, the dispatch/return contract, and the never-delegate list |
...
**Delegate — source digest (content-type files in `sources/` total > ~1,500 lines, or ≥3 source files)**: dispatch one subagent to read them in full and write `<project_path>/analysis/delegates/source_digest.md` — the source outline, the concrete facts each section carries (dates, names, numbers), quotes worth using verbatim, and a `<file>:<heading>` pointer for every entry. This is a **navigation and fact index, not a content channel**: Step 4 reads it to decide the outline and to know where things are, then opens the pointed-at source sections for the literal values it puts on a slide. Dispatch and return contract: [`references/subagent-delegation.md`](references/subagent-delegation.md) §3–§4. Below the threshold, skip it and read the sources directly at Step 4.
...
**Structured-template preflight (before copy)** — **delegate when the workspace has ≥5 template SVGs**: dispatch one subagent to run the inspection below and write its verdict to `<project_path>/analysis/delegates/template_preflight.json` (per-page pass/fail plus the exact offending identities); the main agent decides install-or-stop from that verdict without reading the SVGs. Contract: [`references/subagent-delegation.md`](references/subagent-delegation.md) §1. For every deck/layout workspace, inspect all SVG roots and slots under its normalized template source. Every page must declare root Master/Layout key and picker names; Master/Layout visuals must be direct atoms rather than `<g>`; every slot must be a top-level `<g>` with positive bounds and exactly one compatible carrier, or an explicit composite `object` proxy. A zero-slot Layout is valid. If the SVG package uses a legacy semantic contract, run [`restore-pptx-structure`](workflows/restore-pptx-structure.md) first and return to Step 3 with the migrated workspace. A legacy flat directory shape alone is read compatibility and does not trigger restoration.
...
**Delegate — image acquisition (default whenever any `ai` / `web` / `slice` row still needs the ladder, recovery, or slicing)**: dispatch one subagent that owns this entire Step's execution. It loads `references/image-base.md` plus the path-specific references below, runs the ladder / recovery / slice / `analyze_images.py`, writes back `image_prompts.json` / `image_sources.json`, and writes `<project_path>/analysis/delegates/image_acquisition.md`. The main agent keeps the §VIII plan, the Step 4 manifests, the confirmed image path, and any user-facing web-switch or user-drop offer — the delegate returns `needs_user` for those instead of asking. This keeps `image-generator.md` and `image-searcher.md` out of the main context entirely. Dispatch and return contract: [`references/subagent-delegation.md`](references/subagent-delegation.md) §3–§4. When every row is already terminal from the Step 4 background launch, skip the delegate and just complete the checkpoint.

**Always load the common framework** (main agent when running inline; otherwise the delegate loads it):
...
**Delegate — deck verification (default)**: dispatch one subagent to run the command above, read the printed `_pptx_render/<stem>-grid.png` for the lightweight sanity pass below, open only visibly suspect pages at full resolution, and write `<project_path>/analysis/delegates/deck_verify.md` (exit code, each failed check with its owning artifact, per-page suspicion findings). The delegate never edits `svg_output/` and never re-exports — it returns `ok` / `partial` / `failed` and the main agent owns every fix decision. Dispatch with a `verify-charts` delegate in the same message when the deck has data charts. Contract: [`references/subagent-delegation.md`](references/subagent-delegation.md) §1, §3–§4.
...
| Subagent delegation contract (read at the first delegable step) | `references/subagent-delegation.md` |
